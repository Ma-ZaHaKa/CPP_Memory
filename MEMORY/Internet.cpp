...
//static BOOL ParseURL(PCHAR pszURL, PCHAR pszHost, PCHAR pszObject, bool* bIsHTTPS)
static BOOL ParseURL(const char* pszURL, char* pszHost, char* pszObject, bool* bIsHTTPS)
{
...
//std::string HTTPGetRequest(LPCSTR lpszURL, INT* lpiReturnCode )
//{
...
//	bool bIsHTTPS = false;
//
//	if (!ParseURL((CHAR*)lpszURL, lpszServerName, lpszObjectName, &bIsHTTPS)) return result;
//
//	WORD wPort = bIsHTTPS ? (INTERNET_DEFAULT_HTTPS_PORT) : (INTERNET_DEFAULT_HTTP_PORT);
//	DWORD iFlags = bIsHTTPS ? (INTERNET_FLAG_SECURE | INTERNET_FLAG_RELOAD) : (INTERNET_FLAG_RELOAD);
//
...
std::string HTTPGetRequest(std::string url, INT* lpiReturnCode)
{
	std::string result; // Используем std::string для хранения ответа
